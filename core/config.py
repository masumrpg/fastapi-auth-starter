...
class Settings(BaseSettings):
    ...
    DB_HOST: str = os.getenv('POSTGRESQL_HOST')
    DB_PORT: str = os.getenv('POSTGRESQL_PORT')
    DATABASE_URL: str = f"postgresql+psycopg2://{DB_USER}:%s@{DB_HOST}:{DB_PORT}/{DB_NAME}" % quote_plus(DB_PASSWORD)
    DATABASE_MIGRATION_URL: str = f"postgresql+psycopg2://{DB_USER}:%s@{DB_HOST}:{DB_PORT}/{DB_NAME}" % quote(
        DB_PASSWORD).replace("%", "%%")
